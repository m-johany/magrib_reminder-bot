...
2. **Deploy: mostly DONE (Slg-cf001@sylergy.net account)** — D1 created (`766c58c8-...`, APAC), `account_id` pinned, worker deployed to `https://al-kahf-bot.slg-cf001.workers.dev` (cron */10 active), migrations + seed applied (8 hadiths verified). Johany-account test resources (worker + D1) created earlier in error were deleted. Missing: `wrangler secret put TELEGRAM_BOT_TOKEN` (worker currently 500s with "Empty token!") + webhook registration via `curl setWebhook` (steps in `README.md`).
