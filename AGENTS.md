...
1. **Hadith audit: DONE (2026-08-17)** — all 10 texts verified against primary sources (sunnah.com, Mustadrak al-Hakim, dorar.net, shamela.ws Targhib). 4 PASS as-is; 6 issues found and fixed: wrong citations (H1 → Musnad Ahmad 27516 + al-Nasa'i; H3 → as-Silsilah as-Sahihah 2651 + Sahih at-Targhib 1473; H4 → Muslim 795 only, quote is not Bukhari's wording; H9 → Darimi 3312 + Bayhaqi 5856), duplicate removed (H8 was a truncated copy of H3), and H7's grading corrected (Mundhiri said "acceptable isnad", not hasan; Albani graded it da'if — **H7 keep/drop/replace is an open user decision**). Remote D1 re-seeded to 9 contiguous rows.
2. **Deploy: mostly DONE** — D1 created (`57ff5617-...`, APAC), `account_id` pinned, worker deployed to `https://al-kahf-bot.johany.workers.dev` (cron */10 active), migrations + seed applied. Missing: `wrangler secret put TELEGRAM_BOT_TOKEN` (worker currently 500s with "Empty token!") + webhook registration via `curl setWebhook` (steps in `README.md`).
...
| `db/seed.sql` | 9 hadith (week_order 0–8) |
